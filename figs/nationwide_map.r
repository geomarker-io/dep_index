...
national_map <- leaflet() |>
  ...
  addGlPolygons(data = d,
                fillColor = "dep_index",
                fillOpacity = 0.8, popup = c("dep_index"),
                src = TRUE) |>
  setView(-93.65, 38.0285, zoom = 5) |>
  addScaleBar(position = "bottomright", options =
                scaleBarOptions(metric = TRUE)) |>
  leaflet.extras::addFullscreenControl(position = "bottomleft") |>
  leaflet.extras::addResetMapButton() |>
  leaflet.extras::addControlGPS(options = 
                                  leaflet.extras::gpsOptions(
                                    position = "topleft",
                                    activate = TRUE, 
                                    autoCenter = TRUE,
                                    setView = TRUE))

#mapview::mapshot(national_map, "national_dep_index_map.html")
